...
// Create custom patterns specifically for customer support prompts
const customPatterns: OptimizationPattern[] = [
  {
    id: 'remove-redundant-headers',
    category: 'custom',
    description: 'Remove redundant header prefixes',
    priority: 100,
    preservesFormatting: true,
    find: /## (ROLE AND CONTEXT|KNOWLEDGE BASE|BRAND VOICE AND TONE|RESPONSE STRUCTURE|CAPABILITIES AND LIMITATIONS|PRIORITIZATION FRAMEWORK|ESCALATION PROTOCOL|SPECIAL INSTRUCTIONS|EXAMPLE INTERACTIONS|CONTINUOUS IMPROVEMENT|UPDATES AND MAINTENANCE)/g,
    replace: '## $1'
  },
  {
    id: 'condense-example-interactions',
    category: 'custom',
  ...
  {
    id: 'remove-redundant-descriptions',
    category: 'custom',
    description: 'Remove redundant descriptions in response structure',
    priority: 85,
    preservesFormatting: true,
    find: / - (Demonstrate|Be|Include|Guide|Show)[^-\n]+/g,
    replace: ''
  },
  {
    id: 'condense-bullet-points',
    category: 'custom',
    description: 'Condense bullet points with similar themes',
    priority: 80,
    preservesFormatting: true,
    find: /- ([^\n]+)\n- ([^\n]+)\n- ([^\n]+)\n- ([^\n]+)\n- ([^\n]+)/g,
    replace: '- $1\n- $2\n- $3, $4, $5'
  ...
  {
    id: 'remove-redundant-phrases',
    category: 'custom',
    description: 'Remove redundant phrases',
    priority: 75,
    preservesFormatting: true,
    find: /Your primary role is to provide helpful, accurate, and professional support to our customers while representing our brand values of efficiency, reliability, and transparency./g,
    replace: 'Provide helpful, accurate support representing our values: efficiency, reliability, transparency.'
  },
  {
    ...
    priority: 70,
    preservesFormatting: true,
    find: /If a customer inquiry requires human intervention, follow this procedure:/g,
    replace: 'For inquiries needing human help:'
  },
  {
    id: 'condense-continuous-improvement',
    category: 'custom',
    description: 'Condense continuous improvement section',
    priority: 65,
    preservesFormatting: true,
...
const optimizer = new PromptOptimizer({
  model: 'gpt-3.5-turbo',
  aggressiveness: 'medium', // Using medium to avoid over-optimization
