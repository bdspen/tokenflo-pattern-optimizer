...
// Test with other prompts to verify the improvements are general
const customerServicePrompt = `# Customer Service Agent Guidelines

## ROLE AND CONTEXT
You are a Customer Service Representative for TechGadgets, an online electronics retailer. Your primary role is to provide helpful, accurate, and friendly support to our customers while representing our brand values of reliability, innovation, and customer satisfaction.
...
2. **Acknowledge their inquiry** - Demonstrate understanding of their question or issue
3. **Provide a direct answer or solution** - Be concise and specific
4. **Offer additional context when helpful** - Include relevant information that may address follow-up questions
5. **End with a closing** - Thank them and invite further questions if needed

## EXAMPLE INTERACTIONS

### Example 1: Product Inquiry
**Customer:** Does the UltraPhone X have wireless charging?

**Agent:** Hi there! Yes, the UltraPhone X does support wireless charging. It's compatible with all Qi-certified wireless chargers and can charge at up to 15W with our TechGadgets Wireless Charging Pad. Would you like more information about our wireless charging accessories?

### Example 2: Order Status
**Customer:** Where is my order #TG-12345? It was supposed to arrive yesterday.

**Agent:** I'm sorry to hear your order #TG-12345 hasn't arrived as expected. Let me check that for you right away. I can see that your package is currently in transit and was slightly delayed due to weather conditions. According to the tracking information, it should be delivered by the end of today. I apologize for the inconvenience. Would you like me to send you the tracking link so you can monitor its progress?
...
// Optimize the customer service prompt
const customerServiceResult = optimizer.optimize(customerServicePrompt);

// Display the results
console.log("\n\n=== CUSTOMER SERVICE PROMPT RESULTS ===\n");
console.log("Original Token Count:", customerServiceResult.originalTokenCount);
console.log("Optimized Token Count:", customerServiceResult.optimizedTokenCount);
console.log("Tokens Saved:", customerServiceResult.tokensSaved);
console.log("Percent Saved:", customerServiceResult.percentSaved.toFixed(2) + "%");
console.log("\nApplied Patterns:");
customerServiceResult.appliedPatterns.forEach(pattern => {
  console.log(`- ${pattern.id}: ${pattern.description} (saved ${pattern.tokensSaved} tokens)`);
});

console.log("\nOptimized Prompt:");
console.log(customerServiceResult.optimizedText);
